...
// - Limite: max 4 TROUS visibles à l'écran simultanément
...
function spawnHole(){
  if(!gameActive) return;
  // Limite: maximum 4 trous visibles
  if (countVisibleHoles() >= 4) return;

  const minW = 60, maxW = 120;
...
function spawnCat(){
  ...
  const x = camX + canvas.width + distance;
  const w = 64, h = 48;
  cats.push({x, y: baseGroundY - h, w, h, vy:0, vx:-2.6}); // droite -> gauche
}

// Intervalles (respectent gameActive via les checks dans les fonctions)
setInterval(spawnBone, 500);
setInterval(spawnHole, 1100);
